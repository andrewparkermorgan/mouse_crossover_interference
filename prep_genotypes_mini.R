## convert raw MiniMUGA genotypes to R/qtl2 cross objects for backcross offspring from PAE experiment
...
## raw Mini data for founers of this cross
founders <- readRDS("~/Dropbox/PAE/genotypes/minimuga_founders.raw.rds")
founders <- do_updates(founders, mm_mini)
founders <- rename_individuals(founders, c("F","G","H","V"))
saveRDS(founders, "geno/mini_founders_preclean.rds")

## raw Mini data for backcross offspring, split by cross
crosses <- readRDS("~/Dropbox/PAE/genotypes/minimuga.raw.rds")
crosses <- lapply(crosses, do_updates, attr(founders, "map"))
crosses <- lapply(crosses, function(f) f[ rownames(founders), ] )
saveRDS(crosses, "geno/crosses_mini_preclean.rds")
...
## global genotype QC
prop_missing <- p_missing(founders)
#prop_maf <- calc_maf(all_geno_raw)

# note: we only want autosomes for crossover analysis
mk_non_autos <- subset(mm_mini, !is_autosome(chr))$marker
...
writeLines(mk_bad, "qc/drop_markers_mini.txt")
...
clean_cross_genotypes <- function(dam, sire) {
	...
	writeLines(sus_mk, paste0("qc/drop_per_cross_mini_",dam,sire,".txt"))
...
x2 <- clean_cross_genotypes("F","H")
saveRDS( c(x1,x2), "geno/crosses_mini_clean.rqtl.rds" )
